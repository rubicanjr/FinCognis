/**
 * Canavar Skill Tracker - Stop hook
 * Session sonunda agent-events.jsonl ve error-ledger.jsonl'den
 * skill-matrix.json'i gunceller.
 */
...
// Dosya uzantisindan skill kategorisi cikar
function fileToSkill(detail: string): string | null {
...
function main() {
  ...
  // Mevcut matrix'i oku
  let matrix: SkillMatrix = { agents: {}, updated_at: '' };
  if (existsSync(matrixPath)) {
    try { matrix = JSON.parse(readFileSync(matrixPath, 'utf-8')); } catch { /* fresh */ }
  }
  ...
  // Agent bazli hatalari grupla (main dahil - tum hatalar onemli)
  const agentErrors = new Map<string, ErrorEntry[]>();
  ...
  // Her agent icin profil guncelle
  const allAgentTypes = new Set([...agentEvents.keys(), ...agentErrors.keys()]);
  ...
  for (const agentType of allAgentTypes) {
    ...
    if (taskEvents.length > 0 || errors.length > 0) {
      ...
      // unknown durumu: task yok AMA error de yok → sayma
    ...
    // Skill basari oranlarini errors ile guncelle
    for (const err of errors) {
      const skill = fileToSkill(err.file);
      if (skill && profile.skills[skill]) {
        // Hata varsa success olarak sayma
      } else if (skill) {
        if (!profile.skills[skill]) {
          profile.skills[skill] = { attempts: 0, successes: 0, rate: 0 };
        }
    ...
    for (const [skillName, stats] of Object.entries(profile.skills)) {
      ...
      stats.rate = stats.attempts > 0
        ? Number((stats.successes / stats.attempts).toFixed(2))
        : 1;
    }

    // Common errors guncelle (tum zamanlar)
    const errorPatterns = allErrors
    ...
    profile.common_errors = [...patternCounts.entries()]
      .sort((a, b) => b[1] - a[1])
      .slice(0, 3)
