/**
 * Canavar Cross-Review - PostToolUse hook (Agent)
 * Bir producer agent is bitirince review hatirlatmasi enjekte eder.
 * Yeni agent spawn ETMEZ - mevcut akisa systemMessage ekler.
 */
...
// Reviewer agent'lar: sonsuz dongu onlemi
const REVIEWER_AGENTS = new Set([
  'code-reviewer', 'security-reviewer', 'go-reviewer',
  'python-reviewer', 'plan-reviewer', 'review-agent',
]);
...
function main() {
  ...
  // Sadece producer agent'larda tetikle
  if (!PRODUCER_AGENTS.has(agentType)) {
    console.log('{}');
    return;
  }

  // Reviewer agent'in ici mi? Sonsuz dongu onle
