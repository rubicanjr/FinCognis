  ...
  return (
    <header className="relative flex min-h-[795px] flex-col items-center justify-center overflow-hidden px-6 pb-24 pt-32">
      ...
      <div className="relative z-10 mx-auto max-w-5xl text-center">
        <div className="mb-8 inline-flex items-center gap-2 rounded-full border border-outline-variant/20 bg-surface-container-highest px-3 py-1 animate-fade-in-up">
        ...
        <h1 className="animate-fade-in-up-delay-1 mb-6 font-headline text-5xl font-extrabold leading-[1.1] tracking-tighter text-on-surface sm:text-6xl md:text-8xl">
          Hassas Finansal <span className="text-secondary">Zeka</span>
        </h1>
        ...
        <p className="animate-fade-in-up-delay-2 mx-auto mb-10 max-w-2xl font-body text-lg leading-relaxed text-on-surface-variant sm:text-xl md:text-2xl">
          Yuksek frekansli analitik ve kurumsal duzeyde guvenlik kasasi. Fintech operasyonlarinizi mutlak guvenle olceklendirin.
        </p>
      ...
      <div className="animate-fade-in-up-delay-3 relative mt-20 mx-auto w-full max-w-6xl">
        <div className="glass-panel overflow-hidden rounded-2xl border border-outline-variant/20 p-3 shadow-2xl sm:p-4">
          <div className="w-full select-none overflow-hidden rounded-xl" aria-label="FinCognis Dashboard Onizleme" style={{ background: "#0c0e12" }}>
            ...
            <div className="flex min-h-[340px] sm:min-h-[420px]">
              <div className="hidden w-48 shrink-0 flex-col gap-1 border-r border-[#45474b]/30 p-3 sm:flex">
                ...
                {[
                  { icon: "dashboard", label: "Genel Bakis", active: true },
                  { icon: "candlestick_chart", label: "Piyasalar" },
                  { icon: "account_balance_wallet", label: "Portfoy" },
                  { icon: "swap_horiz", label: "Islemler" },
                  { icon: "analytics", label: "Analitik" },
                  { icon: "shield", label: "Guvenlik" },
                ].map((item) => (
                  <div
                    key={item.label}
                    className={`flex items-center gap-2.5 rounded-lg px-2.5 py-2 font-label text-xs transition-colors ${
                      item.active ? "bg-secondary/10 font-semibold text-secondary" : "text-[#8f9095] hover:text-on-surface-variant"
                    }`}
                  >
                    <span className="material-symbols-outlined text-[16px]" style={{ fontVariationSettings: item.active ? "'FILL' 1" : "'FILL' 0" }}>
                      {item.icon}
                    </span>
                    {item.label}
                  </div>
              ...
              <div className="flex-1 space-y-4 overflow-hidden p-4 sm:p-5">
                <div className="grid grid-cols-2 gap-3 lg:grid-cols-4">
                  {[
                    { label: "Toplam Varlik", value: "₺2.847.320", change: "+12.4%", up: true, icon: "account_balance" },
                    { label: "Gunluk Hacim", value: "₺184.5K", change: "+8.2%", up: true, icon: "trending_up" },
                    { label: "Acik Pozisyon", value: "24", change: "-3", up: false, icon: "swap_vert" },
                    ...
                    <div key={card.label} className="rounded-xl border border-[#45474b]/30 p-3" style={{ background: "#191c1f" }}>
                      ...
                      <div className={`mt-1 font-label text-[10px] ${card.up ? "text-[#28c840]" : "text-[#ffb4ab]"}`}>{card.change}</div>
                    </div>
                  ))}
                </div>
                ...
                <div className="flex flex-col gap-3 lg:flex-row">
                  <div className="flex-1 rounded-xl border border-[#45474b]/30 p-4" style={{ background: "#191c1f" }}>
                    <div className="mb-4 flex items-center justify-between">
                      <span className="font-label text-xs uppercase tracking-wider text-[#8f9095]">Portfoy Performansi</span>
                      <div className="flex gap-1.5">
                        {["1G", "1H", "1A", "1Y"].map((t) => (
                          <span
                            key={t}
                            className={`rounded px-2 py-0.5 font-label text-[10px] ${
                              t === "1A" ? "bg-secondary/15 font-semibold text-secondary" : "text-[#8f9095]"
                            }`}
                          >
                            {t}
                          </span>
                  ...
                  <div className="flex w-full flex-col items-center justify-center rounded-xl border border-[#45474b]/30 p-4 lg:w-44" style={{ background: "#191c1f" }}>
                    <span className="mb-3 font-label text-[10px] uppercase tracking-wider text-[#8f9095]">Dagilim</span>
