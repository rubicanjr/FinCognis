...
function correlationBg(value: number): string {
  if (value >= 0.75) return "bg-red-500/30";
  if (value >= 0.5) return "bg-amber-500/25";
  if (value >= 0.2) return "bg-blue-500/20";
  if (value > -0.2) return "bg-emerald-500/15";
  if (value > -0.5) return "bg-cyan-500/20";
...
export function PortfolioHeatmap({ assetIds, heatmap }: HeatmapProps) {
  ...
  return (
    <div className="overflow-x-auto rounded-2xl border border-outline-variant/10 bg-surface-container-low p-3">
      <table className="min-w-[640px] w-full text-xs">
        <thead>
          <tr>
            <th className="px-2 py-2 text-left text-on-surface-variant">Varlik</th>
              ...
              {assetIds.map((colId) => {
                ...
                return (
                  <td key={colId} className="px-1 py-1">
                    <div className={`rounded-md px-2 py-1 text-center font-semibold ${correlationBg(value)}`}>
                      {value.toFixed(2)}
                    </div>
                  </td>
                );
...
export function ContagionMap({ contagion }: ContagionMapProps) {
  ...
  return (
    <div className="grid gap-3 lg:grid-cols-[380px_minmax(0,1fr)]">
          ...
          {layout.map((node) => {
            ...
            return (
              <line
                ...
                x2={node.x}
                y2={node.y}
                stroke={node.totalImpact < 0 ? "#ef4444" : "#22c55e"}
            ...
            return (
              <g key={node.assetId}>
                <circle
                  ...
                  cy={node.y}
                  r={isSource ? 24 : 18}
                  fill={isSource ? "#f97316" : node.totalImpact < 0 ? "#ef4444" : "#22c55e"}
                  fillOpacity={isSource ? 0.9 : 0.72}
                />
                <text x={node.x} y={node.y + 4} textAnchor="middle" fontSize="9" fill="#ffffff" fontWeight="700">
                  {ticker}
                </text>
              </g>
            );
      ...
      <div className="rounded-2xl border border-outline-variant/10 bg-surface-container-low p-3">
        <p className="mb-2 text-xs font-bold uppercase tracking-[0.16em] text-secondary">Bulasicilik Etki Listesi</p>
        <div className="space-y-2">
          {nodes.map((node) => {
            const asset = ASSET_UNIVERSE.find((item) => item.id === node.assetId);
            return (
              <div key={node.assetId} className="rounded-lg bg-surface px-3 py-2">
                <div className="flex items-center justify-between">
                  <p className="text-sm font-semibold text-on-surface">{asset?.ticker ?? node.assetId}</p>
                  <p className={`text-sm font-bold ${node.totalImpact < 0 ? "text-red-400" : "text-emerald-400"}`}>
                    {(node.totalImpact * 100).toFixed(2)}%
                  </p>
                </div>
                <p className="text-[11px] text-on-surface-variant">
                  Direkt: {(node.directImpact * 100).toFixed(2)}% | Yansima: {(node.propagatedImpact * 100).toFixed(2)}%
                </p>
              </div>
            );
          })}
