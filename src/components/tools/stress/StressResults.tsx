  ...
  return (
    <div className="space-y-2">
      {sampled.map((point) => {
        ...
        return (
          <div key={point.day} className="grid grid-cols-[54px_1fr_62px] items-center gap-2">
            <span className="text-[11px] text-on-surface-variant">Gun {point.day}</span>
      ...
      <p className="text-[11px] text-on-surface-variant">
        Mavi bant: P10-P90 dagilimi, cizgi: medyan (P50). Degerler baslangic sermayesinin katsayisidir.
      </p>
    </div>
  );
}
...
export default function StressResults({ analysis, portfolioValue }: StressResultsProps) {
  ...
  return (
    <div className="space-y-4">
      <section className="rounded-2xl bg-surface p-4">
        <div className="grid gap-3 lg:grid-cols-3">
          <div className="rounded-xl bg-surface-container-high p-3 lg:col-span-2">
            <p className="text-[11px] uppercase tracking-[0.14em] text-secondary">Dayaniklilik Skoru</p>
            <div className="mt-1 flex items-end justify-between">
              <p className="font-headline text-3xl font-extrabold text-on-surface">{resilience.toFixed(0)}/100</p>
              <p className="text-xs text-on-surface-variant">Secili kriz: {selected.title}</p>
            </div>
          ...
          <div className="rounded-xl bg-surface-container-high p-3">
            <p className="text-[11px] uppercase tracking-[0.14em] text-on-surface-variant">Secili Kriz Etkisi</p>
            ...
            <p className="mt-2 text-xs text-on-surface-variant">
              Tahmini kayip: {fmtCurrency(Math.max(0, -selected.cumulativeReturn * portfolioValue))}
            </p>
          </div>
        </div>
      </section>
      ...
      <section className="rounded-2xl bg-surface p-4">
        <p className="mb-3 text-xs font-bold uppercase tracking-[0.18em] text-secondary">Crisis Replay Kutuphanesi</p>
          ...
          {analysis.crisisReplayLibrary.map((scenario) => (
            <article key={scenario.scenarioId} className="rounded-xl bg-surface-container-high p-3">
              ...
              <p className={`mt-1 text-sm font-bold ${scenario.cumulativeReturn < 0 ? "text-red-400" : "text-emerald-400"}`}>
                Portfoy: {fmtSignedPct(scenario.cumulativeReturn)}
              </p>
              ...
              <div className="mt-2 space-y-1 text-[11px] text-on-surface-variant">
                <p>BIST100 farki: {fmtSignedPct(scenario.benchmarkComparison.bist100)}</p>
                <p>Altin farki: {fmtSignedPct(scenario.benchmarkComparison.gold)}</p>
                <p>S&P500 farki: {fmtSignedPct(scenario.benchmarkComparison.sp500)}</p>
              </div>
            </article>
          ))}
        </div>
        ...
        <div className="grid gap-3 sm:grid-cols-3">
          {analysis.scenarioSet.map((scenario) => (
            <div key={scenario.label} className="rounded-xl bg-surface-container-high p-3">
              ...
              <p className="mt-1 text-xs text-on-surface-variant">Taban sok: {fmtPct(scenario.shock, 0)}</p>
              <p className="mt-2 text-sm font-bold text-red-400">{fmtCurrency(scenario.stressedLoss)}</p>
              <p className="text-[11px] text-on-surface-variant">Kalan deger: {fmtCurrency(scenario.remainingValue)}</p>
            </div>
          ))}
        </div>
      </section>
      ...
      <section className="grid gap-4 lg:grid-cols-2">
        <div className="rounded-2xl bg-surface p-4">
          <p className="mb-3 text-xs font-bold uppercase tracking-[0.18em] text-secondary">DCC-GARCH + Sira Riski</p>
          <div className="grid gap-3 sm:grid-cols-2">
            ...
            />
            <MetricTile label="Kosullu Volatilite" value={fmtPct(analysis.dccGarch.condVolPortfolio)} />
            <MetricTile label="Median Terminal Servet" value={`${analysis.sequenceRisk.medianTerminalWealth.toFixed(2)}x`} />
            <MetricTile
              label="Depletion Olasiligi"
        ...
        <div className="rounded-2xl bg-surface p-4">
          <p className="mb-3 text-xs font-bold uppercase tracking-[0.18em] text-secondary">Faktor Duyarliligi + PCA</p>
          ...
          <div className="mt-3 rounded-xl bg-surface-container-high px-3 py-2">
            <p className="text-xs text-on-surface-variant">En zayif halka</p>
            <p className="text-sm font-semibold text-red-400">{analysis.factorSensitivity.weakestFactor.toUpperCase()}</p>
            <p className="mt-1 text-xs text-on-surface-variant">
              PCA ilk bilesen aciklama orani: {fmtPct(analysis.factorSensitivity.pcaVarianceExplained)}
            </p>
          </div>
        </div>
      </section>
      ...
      <section className="grid gap-4 lg:grid-cols-3">
        <div className="rounded-2xl bg-surface p-4">
          ...
          <MetricTile label="Agirlikli Tasfiye Gunu" value={analysis.liquidityStress.weightedLiquidationDays.toFixed(1)} />
          <MetricTile label="Slippage Kaybi" value={fmtCurrency(analysis.liquidityStress.slippageLoss)} tone="text-amber-400" />
          <MetricTile label="Gap Kaybi" value={fmtCurrency(analysis.liquidityStress.gapLoss)} tone="text-red-400" />
          <MetricTile
        ...
        <div className="rounded-2xl bg-surface p-4">
          ...
          />
          <MetricTile label="Kupiec p-degeri" value={analysis.validation.kupiecPValue.toFixed(3)} />
        ...
        <div className="rounded-2xl bg-surface p-4">
          <p className="mb-3 text-xs font-bold uppercase tracking-[0.18em] text-secondary">Regulasyon Uyumu</p>
          <MetricTile label="Basel III sermaye ihtiyaci" value={fmtPct(analysis.regulation.baselCapitalNeedPct)} />
          <MetricTile label="CCAR stres kaybi" value={fmtPct(analysis.regulation.ccarLossPct)} />
          <MetricTile label="Solvency II sermaye ihtiyaci" value={fmtPct(analysis.regulation.solvencyCapitalNeedPct)} />
          <div className={`mt-3 rounded-xl px-3 py-2 text-sm font-semibold ${analysis.regulation.compliant ? "bg-emerald-500/15 text-emerald-300" : "bg-red-500/15 text-red-300"}`}>
            {analysis.regulation.compliant ? "Kurumsal uyum siniri icinde." : "Sermaye tamponu yetersiz, yeniden dengeleme gerekli."}
          </div>
        </div>
      </section>
      ...
      <section className="rounded-2xl bg-surface p-4">
        <p className="mb-3 text-xs font-bold uppercase tracking-[0.18em] text-secondary">
          Aksiyona Donusen Ciktilar (Yatirim tavsiyesi degildir)
        </p>
        <div className="grid gap-3 lg:grid-cols-2">
          <div className="rounded-xl bg-surface-container-high p-3">
            <p className="text-sm font-semibold text-on-surface">Hedge ve optimizasyon onerileri</p>
          ...
          <div className="rounded-xl bg-surface-container-high p-3">
            <p className="text-sm font-semibold text-on-surface">Otomatik IPS Taslagi</p>
